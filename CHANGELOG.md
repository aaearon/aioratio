...
- Module-level loggers (`aioratio.auth`, `aioratio.client`, `aioratio.token_store`,
  `aioratio.transport`) emit DEBUG-level events for login, refresh, 401 retry,
...
- **(BC)** `cpms_options(serial)` now only returns `[]` for HTTP 403/404
  ...
  non-403/404 errors will need to add explicit error handling.
...
- `RatioClient.user_id()` now caches the decoded `sub` claim per ID token to
  avoid double-loading the token store on every authenticated call.
